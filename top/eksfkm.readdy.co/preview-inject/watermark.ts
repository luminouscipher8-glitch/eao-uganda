...
const getBasePath = () => (globalThis as any).process?.env?.NEXT_PUBLIC__BASE_PATH__ || "";
...
function initWatermark() {
  ...
  const apiUrl = `https://${(globalThis as any).__READDY_AI_DOMAIN__ || 'api.readdy.ai'}/api/public/user/is_free`;
  const projectId = (globalThis as any).__READDY_PROJECT_ID__ || 'default';
  ...
  function createLogoImage(): HTMLImageElement {
    ...
    applyStyles(img, { width: "92px", height: "auto", paddingLeft: "8px", flexShrink: "0" });
    return img;
  }
  ...
  function createMobileContent(container: HTMLDivElement) {
    ...
    const textPre = document.createElement("div");
    textPre.textContent = texts.prefix;
    applyTextStyle(textPre, "6px");

    row1.append(textPre, createLogoImage());

    // Row 2: suffix text
    const row2 = document.createElement("div");
    row2.textContent = texts.suffix;
    applyTextStyle(row2);
    applyStyles(row2, { textAlign: "center", marginBottom: "12px" });

    container.append(row1, row2, createGenerateButton(false));
  }
